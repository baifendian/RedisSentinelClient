...
RedisClient client("192.168.2.134:26379");
...
TEST(RedisClientTest, mget2)
{
	...
	vector<string> ret = client.mget2(keys);
...
TEST(RedisClientTest, mget3)
{
	...
	for (int i=1; i<5; i++)
	{
		string key;
		key = string("aa") + int2string(i);
		EXPECT_STREQ(kv.kvs[key].c_str(), (string("bb") + int2string(i)).c_str());
	}
}
...
TEST(RedisClientTest, spop)
{
	...
	string ret = client.spop("sadd", "saddv");
...
{
	client.sadd("sadd", "saddv");
	string ret = client.srandmember("sadd", "saddv");
	EXPECT_STREQ(ret.c_str(), "saddv");
}

TEST(RedisClientTest, scard)
{
	int ret = client.scard("sadd", "saddv");
	EXPECT_EQ(ret, 1);
}
...
{
	vector<string> ret = client.smembers("sadd", "saddv");
	EXPECT_EQ(ret.size(), 1);
